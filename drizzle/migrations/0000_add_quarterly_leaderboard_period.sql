...
BEGIN
  ...
  ELSIF _period = 'quarterly' THEN
    start_date := date_trunc('quarter', today_sp)::date;
  ELSE
...
CREATE OR REPLACE FUNCTION app_private.get_trainer_leaderboard(_caller uuid, _period text DEFAULT 'monthly'::text)
 RETURNS TABLE(trainer_id uuid, trainer_name text, active_students integer, avg_points numeric, avg_streak numeric)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
