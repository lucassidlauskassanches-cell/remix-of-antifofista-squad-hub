...
DROP FUNCTION IF EXISTS public.get_trainer_leaderboard(text);

CREATE OR REPLACE FUNCTION app_private.get_leaderboard(_caller uuid, _period text)
RETURNS TABLE (
  student_id uuid,
  full_name text,
  trainer_id uuid,
...
BEGIN
  ...
  is_admin := app_private.has_role(_caller, 'admin'::app_role);
  is_trainer := app_private.has_role(_caller, 'treinador'::app_role);
  ...
  WHERE
    is_admin
    OR (is_trainer AND r.t_id = _caller)
    OR ((NOT is_admin) AND (NOT is_trainer) AND r.s_id = _caller)
  ORDER BY r.pos ASC, r.s_name ASC;
END;
$$;

REVOKE ALL ON FUNCTION app_private.get_leaderboard(uuid, text) FROM PUBLIC;

CREATE OR REPLACE FUNCTION app_private.get_trainer_leaderboard(_caller uuid, _period text DEFAULT 'monthly')
RETURNS TABLE (
  trainer_id uuid,
  trainer_name text,
...
REVOKE ALL ON FUNCTION app_private.get_trainer_leaderboard(uuid, text) FROM PUBLIC;
